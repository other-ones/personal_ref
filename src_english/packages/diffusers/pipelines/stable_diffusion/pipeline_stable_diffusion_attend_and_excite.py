...
class AttendExciteAttnProcessor:
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None,attn_mod_params=None):
        ...
        # print(attn_mod_params is not None,'attn_mod_params in AttendExcideAttnProcessor')
        ...
        if is_cross:
            ...
            pos_masks_batch=pos_masks_batch.repeat(num_heads*cfg_mult,1,1)
            ...
            # neg_masks_batch=neg_masks_batch.repeat(num_heads*cfg_mult,dim=0)
            # SINGLEBATCH
            ...
            if do_classifier_free_guidance:
                ...
                # min_value=attention_scores.min(-1)[0].unsqueeze(-1).repeat(1,1,num_tokens)
                max_value=attention_scores.max(-1)[0].unsqueeze(-1).repeat(1,1,num_tokens)
                offset_pos=max_value-attention_scores
                # offset_neg=cond_scores-min_value 
                # Update scores
                if treg_pos:
                    # print(attention_scores.shape,'attention_scores.shape')
                    attention_scores=attention_scores+((offset_pos*(pos_masks_batch>0))*(treg_pos))
                ...
                # attention_scores[(batch_size*num_heads):]=attention_scores
        # Attn Modulation
