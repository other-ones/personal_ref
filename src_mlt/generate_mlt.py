...
def main(args):
    ...
    if (hostname == 'ubuntu' or hostname.startswith('Qlab')):
        exp_name=args.resume_unet_ckpt_path.split('/')[-3]
        unet_file=args.resume_unet_ckpt_path.split('/')[-1]
        unet_file_name=unet_file.split('.')[0]
        step_name=unet_file_name.split('_s')[-1]
        step=int(step_name)


        ckpt_name=args.resume_unet_ckpt_path.split('/')[-1].split('.')[0]
        stepname=ckpt_name.split('_')[-1]
        exp_dir=os.path.join(args.output_dir,exp_name+"_{}".format(stepname))
        codepath=os.path.join(exp_dir,'src')
        os.makedirs(codepath,exist_ok=True)
        os.system('cp *.py {}'.format(codepath))
        os.system('cp *.sh {}'.format(codepath))
    ...
    if (hostname == 'ubuntu' or hostname.startswith('Qlab')):
        ...
        rendering_dir=os.path.join(exp_dir,'renderings')
    ...
    meta_path=os.path.join(exp_dir,'meta.txt')
    ...
    with torch.no_grad():
        for step, batch in enumerate(test_dataloader):
            ...
            image_list = pipeline(prompt=prompt, 
                                  spatial_render=spatial_renderings_rgb, masks_tensor=masks_tensor,
