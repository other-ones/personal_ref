...
class OCRDatasetMLZeroShot(Dataset):
    """
    ...
    def __init__(
        ...
        self.mario_db = mario_db
        
        self.latin_langs=['english','french','german','italian']
        self.script_info = script_info
        self.synth_ratio = synth_ratio
        self.target_scripts=list(script_info.target_scripts)
        self.mario_prob=mario_prob
        ...
        self.EOS=0
        self.caption_lines={}
        self.instance_data_list=[mario_db]
        self.latin_dbs=[]
        if 'latin' in self.target_scripts:
            self.instance_data_list+=['synth_pexel_latin3']
        if 'hindi' in self.target_scripts:
            self.instance_data_list+=['synth_pexel_hindi3']
        if 'bengali' in self.target_scripts:
            self.instance_data_list+=['synth_pexel_bengali3']
        if 'thai' in self.target_scripts:
            self.instance_data_list+=['synth_pexel_thai3']
        if 'greek' in self.target_scripts:
            self.instance_data_list+=['synth_pexel_greek3']
        if 'russian' in self.target_scripts:
            self.instance_data_list+=['synth_pexel_russian3']
        # if 'russian' in self.target_scripts:
        #     assert False
            # self.instance_data_list+=['synth_pexel_russian3']
        print(self.instance_data_list,'self.instance_data_list')
        if self.synth_ratio<1:
            self.instance_data_list+=['icdar2019_matched']
        # icdar2019_matched-synth_pexel_thai3-synth_pexel_greek3-synth_pexel_german3-synth_pexel_italian3-synth_pexel_french3-synth_pexel_hindi3-synth_pexel_bengali3
    ...
    def get_icdar_idx(self,sampled_script):
        """
        00001 - 01000:  arabic
        01001 - 02000:  english
        02001 - 03000:  french
        ...
        # ['english', 'italian', 'german', 'french'] list(self.script_info.latin_probs.keys())
        # [0.1, 0.25, 0.3, 0.35] list(self.script_info.latin_probs.values())

        # print(list(self.script_info.latin_probs.keys()),'list(self.script_info.latin_probs.keys())')
        # print(list(self.script_info.latin_probs.values()),'list(self.script_info.latin_probs.values())')
        if sampled_script=='latin':
            sampled_lang=np.random.choice(list(self.script_info.latin_probs.keys()),p=list(self.script_info.latin_probs.values()))
        else:
            sampled_lang=sampled_script
        # sampled_lang=np.random.choice(['english','french','german','italian','bengali','hindi'],p=[1/21,2/21,2/21,2/21,1/3,1/3])
    ...
    def __getitem__(self, index):
        ...
        while True:
            if np.random.rand()<self.mario_prob:
                db_name=self.mario_db
                sampled_script='latin'
            else:
                synth_sel=np.random.rand()<self.synth_ratio
                if synth_sel:
                    sampled_script=np.random.choice(list(self.script_info.synth_script_ratios.keys()),p=list(self.script_info.synth_script_ratios.values()))
                    # print(sampled_script,'sampled_script_synth')
                    db_name='synth_pexel_{}3'.format(sampled_script)
                else:
                    # ICDAR19
                    db_name='icdar2019_matched'
                    sampled_script=np.random.choice(list(self.script_info.real_script_ratios.keys()),p=list(self.script_info.real_script_ratios.values()))
                    index,range_script,range_lang=self.get_icdar_idx(sampled_script=sampled_script)
            ...
            for text_label_line in open(text_label_path,encoding='utf-8-sig').readlines():
                ...
                if len(word)>26:
                    continue
                if script not in self.target_scripts: #HERE
                    assert False
            ...
            if len(words_list):
                ...
                break
            index+=1 
        # print(db_name,'db_name',img_path,'img_path')
