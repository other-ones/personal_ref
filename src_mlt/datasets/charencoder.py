...
class CharEmbedderConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_char_seq_length = max_char_seq_length
...
class CharEmbedder(PreTrainedModel):
    ...
    def __init__(self, config: CharEmbedderConfig, **kwargs):
        ...
        print("Initialize char embedder...")
        self.max_char_seq_length = config.max_char_seq_length
        self.embed_dim = config.embedding_dim
        self.char_embedding = nn.Embedding(
            config.vocab_size, config.embedding_dim, padding_idx=config.padding_idx
        )
        self.position_embedding = nn.Embedding(
            config.max_char_seq_length, config.embedding_dim)
        self.register_buffer(
            "position_ids", torch.arange(config.max_char_seq_length).expand((1, -1))
        )
    ...
    def forward(
        ...
        if input_embeds is None:
            ...
            # print(embeds.shape,'embeds.shape',position_embedding.shape,'position_embedding.shape')
...
class LengthEmbedderConfig(PretrainedConfig):
    model_type = "char_embedder"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embedding_dim = embedding_dim
        self.max_length_seq_length = max_length_seq_length
        self.padding_idx = padding_idx
        self.encoder_config = encoder_config


class LengthEmbedder(PreTrainedModel):
    config_class = CharEmbedderConfig
    base_model_prefix = "charembedder"

    def __init__(self, config: CharEmbedderConfig, **kwargs):
        super().__init__(config, **kwargs)
        print("Initialize length embedder...")
        self.max_word_length = config.max_word_length
        self.embed_dim = config.embedding_dim
        self.length_embedding = nn.Embedding(
            config.max_word_length, config.embedding_dim, padding_idx=config.padding_idx
        )
        # self.register_buffer(
    ...
    def forward(
        self, length_ids=None, position_ids=None, input_embeds=None, do_encode=False
    ):
        assert (length_ids is not None) ^ (
            input_embeds is not None
        ), "One and only one of char_input_ids and length_embeds should be provided."
        if input_embeds is None:
            embeds = self.length_embedding(length_ids)
        else:
            embeds = input_embeds
        return (embeds,)
