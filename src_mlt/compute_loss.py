...
def _symmetric_kl(attention_map1, attention_map2):
    ...
    # attention_map1 is not normalized
    ...
    p = dist.Categorical(probs=attention_map1) #inside, the attention_map is normalized by attention_map/=sum(attention_map)
...
def _calculate_outside_loss(attention_maps, src_indices, outside_loss):
    ...
    computed_pairs = set()
    pair_counter = 0
    # attention_maps not normalized to [0,1]
    # for item in attention_maps:
    #     print(torch.max(item),torch.min(item),'attention_maps')
    # exit()
...
def extract_attribution_indices(doc):
    # doc: spacy parser, when print(doc) -> printouts prompt
    # doc = parser(prompt)
    subtrees = []
...
def get_attention_map_index_to_wordpiece(tokenizer, prompt):
    # prompt: silver corn next to blue bananas 
    # tokenizer: CLIPTokenizr
    attn_map_idx_to_wp = {}
    wordpieces2indices = get_indices(tokenizer, prompt)
    # Ignore `start_token` and `end_token` [1:-1]
