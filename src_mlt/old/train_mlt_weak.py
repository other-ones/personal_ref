...
from src_mlt.datasets.script_info_latin import scriptInfo
from datasets.ocr_dataset_ml_weak import OCRDatasetMLWeak, mask_transforms
...
def main(args):
    ...
    train_dataset = OCRDatasetMLWeak(
    ...
    if args.mario_batch_size:
        train_dataset_mario = OCRDatasetMLWeak(
    ...
    for epoch in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            if len(text_boxes):
                ...
                # text_rec_loss_recnet = ce_criterion(text_preds_original.view(-1,text_preds_original.shape[-1]), text_gts.contiguous().view(-1))#*0.05
            ...
            if accelerator.sync_gradients:
                ...
                if ((global_step - 1) % args.inference_steps) == 0:    
                    ...
                    if accelerator.is_main_process:
                        ...
                        # glyph_texts = ["пдвржен","éçlameùr","तेवरमों","περνούτ"]
                        # inference_lang_list=['russian','french','hindi','greek']
                        # glyph_texts_batch = [["περλφύτ"],["بعقد."],["ธนานุบาล"],["äußert"]]
                        # inference_lang_list_batch=[['greek'],["arabic"],["thai"],["german"]]
                        
                        glyph_texts_batch = [["ธนานุบาล"],["περλφύτ"],["तेवरमों"],["অইছিল"],["пдвржен"],["dormìpò"],["défià"],["äußert"],["Purdue"]]
                        inference_lang_list_batch=[['thai'],['greek'],["hindi"],["bengali"],["russian"],["italian"],["french"],["german"],["english"]]
                        ...
                        for gen_idx in range(len(glyph_texts_batch)):
                            ...
                            inference_script_idxs=[script_info.script2idx[script] for script in inference_scripts_list]
