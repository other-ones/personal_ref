...
class Attention(nn.Module):
    ...
    def head_to_batch_dim(self, tensor, out_dim=3):
        # print(tensor.shape,'tensor.shape1')
        # example: (4,4096,320) -> (4,4096,5,64) -> (4,5,4096,64) -> (20,4096,64)
        head_size = self.heads
        batch_size, seq_len, dim = tensor.shape
        tensor = tensor.reshape(batch_size, seq_len, head_size, dim // head_size)
        # print(tensor.shape,'tensor.shape2')
        tensor = tensor.permute(0, 2, 1, 3)
        # print(tensor.shape,'tensor.shape3')
        if out_dim == 3:
            tensor = tensor.reshape(batch_size * head_size, seq_len, dim // head_size)
        # print(tensor.shape,'tensor.shape4')
    ...
    def get_attention_scores(self, query, key, attention_mask=None):
        ...
        # torch.Size([20, 4096, 77]) baddbmm_input.shape
        # torch.Size([20, 4096, 64]) query.shape
        # torch.Size([20, 64, 77]) key.transpose(-1, -2).shape
