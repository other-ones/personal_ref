...
def _find_modules_v2(
    ...
    # print(ancestor_class,'ancestor_class') # ['CLIPAttention'] ancestor_class
    # for module in model.modules():
    #     print(module,'model.module')
    if ancestor_class is not None: #HERE
    ...
    if ancestor_class is not None: #HERE
        if exclude_substring is not None:
            ancestors = (
                ...
                if ((module.__class__.__name__ in ancestor_class) and (exclude_substring not in name))
            )
        else:
            ancestors = (
                module
...
def inject_trainable_lora(
    ...
    for _module, name, _child_module in _find_modules(
        model=model, ancestor_class=target_replace_module, search_class=[nn.Linear],exclude_substring=exclude_substring
    ):
...
def inject_trainable_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        # if name.startswith('to') or name.startswith('time'):
            # continue
